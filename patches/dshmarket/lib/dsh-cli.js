...
function spawnEnv() {
    ...
    // Desktop-packaged runtime: ensure git and pnpm are discoverable.
    // Windows builds bundle node+pnpm under resources/runtime, but PATH may
    // omit them; git for Windows is required for github: source resolution.
    if (process.platform === 'win32') {
        const candidateDirs = [
            process.env.DSH_PNPM_BIN ? dirname(process.env.DSH_PNPM_BIN) : null,
            process.env.DSH_DESKTOP_NODE_BIN ? dirname(process.env.DSH_DESKTOP_NODE_BIN) : null,
            `${process.env.ProgramFiles ?? 'C:\\Program Files'}\\Git\\cmd`,
            `${process.env.ProgramFiles ?? 'C:\\Program Files'}\\Git\\usr\\bin`,
            `${process.env['ProgramFiles(x86)'] ?? 'C:\\Program Files (x86)'}\\Git\\cmd`,
        ].filter(Boolean);
    ...
    const env = { ...process.env, ...proxyEnvForPnpm(process.env, activeRegion()), CI: 'true', PATH: parts.join(separator) };
    // Explicitly preserve desktop runtime variables — proxyEnvForPnpm must not
    // wipe them, and a re-spawned dsh needs DSH_PNPM_BIN to find bundled pnpm.
    for (const key of ['DSH_PNPM_BIN', 'DSH_HOME', 'DSH_DESKTOP_NODE_BIN', 'DSH_PROFILE_SAFE_MODE']) {
...
/**
 * pnpm 11.x enforces integrity verification for URL tarballs, but GitHub
 * codeload tarballs carry no integrity field → ERR_PNPM_MISSING_TARBALL_INTEGRITY.
 * Download the tarball into DSH_HOME/bundled-plugins and return a file: spec.
 */
async function downloadTarballToBundled(url) {
    const dshHome = process.env.DSH_HOME || join(homedir(), '.dsh');
    const bundledDir = join(dshHome, 'bundled-plugins');
    if (!existsSync(bundledDir))
        mkdirSync(bundledDir, { recursive: true });
    const hash = createHash('sha1').update(url).digest('hex').slice(0, 12);
    const ext = url.toLowerCase().endsWith('.zip') ? 'zip' : 'tgz';
    const localPath = join(bundledDir, `url-${hash}.${ext}`);
...
export async function runDshPlugin(profile, pluginArgs) {
    ...
    // pnpm 11.x rejects URL tarballs without integrity (GitHub codeload).
    // Rewrite add targets that are http(s) tarballs to local file: specs.
    // GitHub codeload URLs look like .../tar.gz/<commit> (end with hash, not .tar.gz).
    if (pluginArgs[0] === 'add') {
        const targetIdx = pluginArgs.findIndex(a => /^https?:\/\//.test(a) && !/\.git$/i.test(a) && /(tar\.gz|tgz|zip|codeload\.github\.com|gh-proxy\.com)/i.test(a));
        if (targetIdx !== -1) {
            ...
            try {
                ...
                pluginArgs[targetIdx] = `file:${localPath.replace(/\\/g, '/')}`;
                logEvent('info', 'install', `rewrote URL tarball to local file: ${basename(url)} -> ${localPath}`);
            }
            catch (err) {
                logEvent('error', 'install', `failed to download URL tarball ${url}: ${err.message}`);
            }
        }
    }
