...
import { codeloadAllowBuildsKey, findCatalogEntryForLocal, findInstalledAlias, gitAllowBuildsKey, installTargetFor, isLocalSpec, NPM_NAME_RE, repoOfTarget, restoreBlockedByWorkspace, restoreTargetForLocal, workspaceProtocolDeps } from './sources.js';
            ...
            handler: async (request, response) => {
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        // Capture BEFORE remove — packageHasClientPart reads
                        // node_modules/<name>/package.json, which pnpm deletes during the
                        // remove, so checking after would always return false (#uninstall-refresh).
                        ...
                        // A client-part plugin's UI bundle is still live in the page after
                        // remove — the same refresh signal the toggle route uses, so the
