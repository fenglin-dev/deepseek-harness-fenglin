...
官方 `~/.dsh` 只能导入到独立环境，不再提供直接共享选项。Open DeepSeek Harness Desktop 创建或采用配置目录时会写入专用身份文件；首次启动只允许直接复用具有该身份文件或有效旧版桌面设置记录的社区配置。仅包含通用 DSH 文件的其他社区桌面版目录不会被当成本应用配置，以免加载不兼容的 Profile、插件或桌面状态。
