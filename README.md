...
安装包携带七个启动预设的完整性校验归档及按平台预构建的完整 Profile：插件市场、`dsh-im`、`dsh-skill-picker`、Better Sidebar、`dsh-pocket`、`@ychris12138/dsh-usage-stats` 和 `dsh-smooth-stream`；Usage Stats 提供 Token 用量、Provider 账户、会话费用估算、预算与导出能力，Smooth Stream 为回复中的 Markdown、代码块、表格和工具结果提供平滑流式渲染与滚动。首次准备优先部署完整模板，不需要临时联网或逐个运行插件安装；包内归档仍用于维修和兼容回退。普通传递依赖继续由 Profile 的 pnpm 解析规则管理。
