...
Python 不再随桌面安装包分发。用户可在“设置 → 工具与能力 → 工作运行时”中联网下载与当前完整桌面版本匹配的“Python 与 Office 工具包”；支持 Windows x64、macOS arm64/x64 和 Linux x64。“Python PTC（实验）”在 macOS 与 Linux 上复用同一载荷，但保持独立关闭，只有用户确认模型生成的 Python 会作为新的本机进程运行、且这不是安全沙箱后才可启用。Windows 不显示 PTC 安装操作。
