...
进入客户端后，初始化向导会引导配置模型 API Key、微信或飞书等 IM 机器人，以及按需连接 Codex。所有步骤都可以跳过，并在之后从设置页继续完成。
...
- **跨平台标题栏**：Windows 和 Linux 使用与应用主题一致的自定义标题栏，macOS 保留原生窗口行为。
...
2. 通过预设的 `dsh-im` 配置微信、飞书、钉钉、企业微信、QQ、Slack、Telegram、Discord 或 WhatsApp 机器人。
3. 按需安装并连接官方 Codex 能力。
