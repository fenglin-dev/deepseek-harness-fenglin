...
- Build one locked Python runtime for each native target inside `.github/workflows/desktop-packages.yml`, verify it on that target, and embed only that target's archive and metadata in its installers. The official Office engine remains an on-demand npm package and is not embedded. A missing or invalid Python archive blocks that platform package.
