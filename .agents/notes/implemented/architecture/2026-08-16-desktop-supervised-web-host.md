...
Bundled startup plugins use only integrity-checked local archives. A schema-4 seed marker separates the bundled version already evaluated from the version read from the installed package, and bypasses CLI work only when the Profile declaration, physical package, state and actual version agree. Candidate transactions preserve desktop archive ownership through an explicit active-home mapping. Exact historical registry versions recorded in the bundled manifest migrate to the current archive; custom sources, explicit removal, snapshot holds and newer versions remain user-owned or preserved. Each build approval has a fifteen-second deadline, each archive install has a forty-five-second deadline, and the stage has a two-minute total budget. Every plugin mutation uses its own hidden safety point and offline-frozen rollback; a failed plugin does not write a success marker or block later plugins unless rollback itself fails. A short persisted cooldown prevents one broken version from consuming the same startup budget on every launch.
