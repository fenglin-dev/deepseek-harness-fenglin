...
两个 preload 暴露互不相交的能力。桌面应用 IPC 只接受 Harness 渲染器，最小化、最大化或还原、关闭意图及原生菜单弹出请求只接受标题栏渲染器。导航、新窗口策略和权限处理只应用于 Harness。主进程会把 Harness 页面标题和主题变化同步到标题栏。[应用菜单决策](../feature/2026-09-03-desktop-application-menus.zh.md)通过原生弹出菜单扩展操作，不扩大标题栏视图，也不在其中暴露 Harness 桥。
