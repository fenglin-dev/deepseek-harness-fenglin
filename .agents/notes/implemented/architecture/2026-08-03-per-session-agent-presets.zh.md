...
**发现刻意保持浅层；激活就绪检查则不然。** 解析 preset 并解析其顶层模块名，无法证明某个插件的运行时配置仍符合当前 schema。因此，用户作出选择后，会在创建任何 Session 之前预检 Agent 实际将加入的同一份 standing mount。运行时失败只在组装路径、修改时间和文件大小均未改变时保留为名单中的损坏原因；文件一经编辑即可再次尝试。传输失败不会把 preset 判坏。只有未指名 preset 的新 Session 会在配置默认值失效时回退到随附的 `standard` preset，使一个过期的本地默认值不至于禁用新建会话入口。显式选择和恢复已有 Session 仍严格失败：静默替换前者会歪曲用户请求，替换后者则会让历史在不同工具集合下重放。
