...
The historical `apps/desktop/scripts/prepare-dsh.ts` runtime preparation materialized the production graph once at build time and shipped it through `extraResources/dsh`. The Electron shell stayed in ASAR. A bundled upstream Node process ran the private Desktop Host from resources and loaded enabled plugins from `$DSH_HOME/profiles/desktop`.
