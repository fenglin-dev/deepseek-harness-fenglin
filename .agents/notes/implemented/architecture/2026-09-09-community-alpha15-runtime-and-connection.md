...
The integration target is the `dsh-v0.1.5-rc.2` release tag. The external Harness runtime is Node `24.17.0`; Electron remains `43.2.0` and pnpm remains `11.7.0`. The macOS installer requires `13.5`, matching the external Node binary's deployment requirement. Updating the external runtime does not change Electron's embedded Node.
