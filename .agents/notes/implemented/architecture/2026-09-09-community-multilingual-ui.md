...
Locale tests pin stable ids, regional and underscore matching, English fallback, Unicode and Windows paths, unknown placeholders, malformed cache recovery, and atomic cache contents. Client lifecycle tests pin catalog registration and complete teardown. Feature tests require every shipped dictionary to have the English key set and identical placeholder sets. Strict TypeScript compilation checks additional dictionaries against each feature's `LocaleNamespaceMap` key union.
