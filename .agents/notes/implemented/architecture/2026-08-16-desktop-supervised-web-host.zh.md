...
启动预置插件只使用经过完整性校验的本地归档。schema 4 seed marker 会把已评估的安装包预设版本与从实际安装包读取的版本分开记录；只有 Profile 声明、实体包、状态与实际版本一致时才能跳过 CLI。候选事务通过显式的活动 home 映射保留桌面归档归属。预设清单中经过核验的精确历史 registry 版本会迁移到当前归档；自定义来源、明确卸载、快照版本保持与更高版本继续归用户所有或保持不变。每次构建授权的期限为十五秒，每个归档安装为四十五秒，整个阶段总预算为两分钟。每次插件变更都使用独立的隐藏安全点和离线冻结回滚；单个插件失败不会写入成功 marker，也不会阻塞后续插件，除非回滚本身失败。持久化的短期冷却会避免同一故障版本在每次启动时耗尽相同预算。
