...
The two preloads expose disjoint capabilities. Desktop application IPC accepts only the Harness renderer, while minimize, maximize or restore, close intents, and native menu popup requests accept only the title-bar renderer. Navigation, window-open policy, and permission handling apply only to Harness. Harness page-title and theme changes are copied to the title bar by the main process. The [application-menu decision](../feature/2026-09-03-desktop-application-menus.md) adds native popups without widening the title-bar view or exposing the Harness bridge there.
