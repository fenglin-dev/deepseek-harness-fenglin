...
签名清单及证明文件一起托管在本仓库 GitHub Pages 的 `metadata/external-tools/v1/` 路径下。同一个仅允许 master 的工作流先验证 npm 坐标并签名清单，再上传只含元数据的 Pages 产物；依赖该产物的任务使用 Pages 权限部署，而不使用仓库内容写入权限。社区仓库的文档工作流不能覆盖此站点。发布不会创建、更新或删除 GitHub Release。旧安装包在重新构建前仍使用编译时的 Release 地址；地址不可用时使用已验证缓存或内置坐标，不接受未签名的元数据。
