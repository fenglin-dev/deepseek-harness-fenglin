...
桌面宿主通过窄范围 Electron Bridge 提供版本化诊断演练中心。目录包含十六个固定场景标识和固定样本字节：兼容与不兼容 Host 影子副本、孤立 Bundle、旧版隔离卸载残留、scoped 根包与 unscoped Loader 包名不匹配、入口可解析但内部依赖缺失的 Loader、已安装 Host 依赖缺少插件要求的 API 导出、损坏的设置文档、安装包内 `dsh-font` 客户端不兼容、缺失模块、无效 Patch、重复 Loader、生命周期失败、构建许可被阻止、修复中断，以及有界的启动操作超时。渲染层只能选择这些标识，以及隔离沙箱或需明确确认的当前 Profile。默认不选中任何场景。
