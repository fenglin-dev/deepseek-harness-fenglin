...
Community desktop settings, onboarding, plugin diagnostics and snapshot controls follow the same neutral hairline rules. The selection-action menu uses the prominent elevation token with an l1 stroke; the imported-plugin restore dialog uses the panel token. Their state-colored borders and focus behavior remain component-owned.
