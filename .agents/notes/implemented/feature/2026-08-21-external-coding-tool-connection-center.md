...
The packaging gate reads the same compatibility manifest as the embedded fallback and queries the canonical npm registry before any desktop artifact is built. It requires each Provider coordinate and reviewed SHA-512 to match, requires the Provider to depend on the reviewed native product runtime, and resolves every optional platform package to an exact published version with integrity metadata. The [immutable manifest decision](../architecture/2026-09-16-immutable-external-tool-compatibility-manifests.md) binds each signed remote document to one complete desktop version and the exact pins embedded in that installer. A failed or unavailable lookup tries the verified cache, then falls back to those embedded pins; it never follows a dist-tag. The renderer submits only a closed tool id to this resolver and receives no path, command, or general package-resolution capability.
...
The manifest records the source version reviewed for its pins separately from the Provider version. Packaging rejects a stale source-review baseline or runtime mismatch rather than assuming an upstream merge also reviewed desktop compatibility; registry checks still reject unpublished coordinates. Desktop refreshes on each install request and shares only an in-flight lookup, so a later request can recover from an offline failure. Signature, expiry, exact desktop-version, attested-subject, and embedded-pin equality checks apply on every lookup. Tests cover network recovery, rejection of changed coordinates, prerelease isolation, and concurrent requests sharing one refresh.

Version-named manifests and their shared multi-subject attestation are served under the repository's GitHub Pages `metadata/external-tools/v2/` path. The master-only workflow verifies npm coordinates, refuses to replace different content already published at a version URL, signs every retained manifest, and uploads a metadata-only Pages artifact; a dependent job deploys it with Pages permission rather than repository-content write permission. The community repository's documentation workflow cannot deploy over this site. Publication does not create, update, or delete GitHub Releases. Clients compiled against an older endpoint use their verified cache or embedded pins when that endpoint is unavailable.
...
**Update a released client's Provider coordinates through a rolling manifest.** Superseded by the [immutable manifest decision](../architecture/2026-09-16-immutable-external-tool-compatibility-manifests.md), because sharing one compatibility line across prereleases lets an older client accept coordinates reviewed only with a newer desktop build.
