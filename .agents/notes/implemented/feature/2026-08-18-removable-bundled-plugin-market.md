...
macOS, Windows, and Linux artifacts carry one shared resolved snapshot of eight audited startup archives and six diagnostic resources. Packaging requires npm registry access, but preset installation in the released application does not. A local packaging command refreshes registry-backed archives unless `DSH_BUNDLED_PLUGINS_REFRESH=0` requests verification of a pre-resolved snapshot. Better Sidebar and Mermaid add their profile installation time before Harness startup; Codex and Claude Code consume none until requested. Tests cover stable-version rejection, archive integrity, atomic snapshot replacement, local-only installation, ownership-aware adoption and upgrade, downgrade and uninstall suppression, single-flight jobs, staged progress, bounded diagnostics, and renderer fallback. The installed Windows smoke requires all eight startup dependencies before artifact upload, while online-only external tools must remain absent before user action.
