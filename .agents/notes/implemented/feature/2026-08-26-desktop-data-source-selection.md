...
After first-run setup, General Settings and the native application-menu destination reopen the same chooser rather than exposing a second switch-only modal. Runtime entry adds a localized **Back to main page** action; closing the chooser, pressing Escape, or selecting that action cancels without changing the active home and restores focus to the main window. Repeated requests focus the one existing chooser. The renderer still receives only short-lived opaque source and destination identifiers. Electron revalidates them immediately before preparing the selected import, copy, direct use, or fresh setup, then atomically replaces the setup record and requests a full application restart. A copy request cannot use the running active home as its source. The app-owned default destination is offered only when it is empty and distinct from the active home. Custom destinations that equal, contain, or are contained by the active home are rejected before any data or identity file is written. A new empty-folder home uses the durable `created` setup mode and follows the ordinary Profile repair and bundled-plugin seeding flow. The prior home is never moved, overwritten, or deleted. A launch-time explicit `DSH_HOME` keeps authority and hides the control.
