...
describe('durable V3 admission failures', () => {
  it('preserves the community external-tool audit without treating turn and step as sequence references', () => {
    const audit = event('external-tools/resolved', { turn: 1, step: 1, tools: ['codex', 'claude-code'] }, { ignorable: true })
    const source = [...opening, audit]
    const before = JSON.stringify(source)
    const output = migrate(source)
    const migrated = output.events.find(value => value.type === 'external-tools/resolved')
    expect(migrated).toEqual({ ...audit, seq: 3 })
    expect(JSON.stringify(source)).toBe(before)
    expect(() => restoreReleasedV3Artifact(output, new Set(['external-tools/resolved']))).not.toThrow()
  })

  it.each([
    { turn: 1, step: 1, tools: ['unclassified-provider'] },
    { turn: 1, step: 1, tools: ['codex'], sourceEventSeq: 0 },
  ])('rejects unaudited external-tool payloads without broadening migration admission: %j', (data) => {
    expect(() => migrate([...opening, event('external-tools/resolved', data)])).toThrow()
  })
