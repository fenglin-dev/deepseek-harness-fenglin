...
class DecodedReleasedV1ToV2Stage extends TransformedReleasedV1ToV2Stage {
  private readonly emptyTools: LegacyEmptyTools
  ...
  override transformEvent(
    ...
    this.emptyTools.accept(event, (item) => {
      if (item.type !== 'assistant/chunk'
        && RELEASED_V0_EVENT_DISPOSITIONS[item.type] !== undefined) {
        assertReleasedEventPayload(item, 1)
      }
      super.transformEvent(item, context)
    })
  }

  override transformRun(run: SessionFormatEventRun, context: SessionFormatMigrationContext): void {
    this.emptyTools.finish()
    super.transformRun(run, context)
  }

  override finish(context: SessionFormatMigrationContext): number {
    this.emptyTools.finish()
