...
describe('HMR exact config paths', () => {
  ...
  it('serializes refreshes and waits for them during disposal', async () => {
    ...
    let active = 0
    let maxActive = 0
    const dispose = await watchConfig(ctx, filename, { interval: 10 }, async () => {
      active += 1
    ...
    writeFileSync(join(dir, 'unrelated.yml'), 'ignored')
  ...
  it('releases registration after initial file inspection fails', async () => {
    ...
    const inspect = vi.mocked(fsPromises.stat)
    const native = inspect.getMockImplementation()!
    inspect.mockImplementationOnce(native).mockRejectedValueOnce(failure)
  ...
  it('logs a normalized refresh failure and continues processing later events', async () => {
    ...
    await watchConfig(ctx, filename, { interval: 10 }, () => {
      if (++calls === 1) throw 42
