...
Before mounting profile rows, the `dsh` launcher computes one immutable package-resolution generation from the installation and ordered bundle dependency graphs. Runtime mode is the default: it installs the generation through Node's ESM and CommonJS resolvers rather than using fallback links for runtime imports. Plain Node callers of `runProfile` may explicitly select link mode to materialize the generation, dual mode to materialize and verify it, or runtime mode. A profile that is also an npm project still maintains the shared fallback anchor before dependency-health checks in every mode. That maintenance waits up to ten seconds for an earlier launcher to finish publishing the shared generation before reporting its writer lock for recovery. Packaged executables and the Electron Host always use runtime mode.
