...
describe('session-log-download browser plugin', () => {
  it('provides one controller and removes its Header contribution on disposal', async () => {
    ...
    const entry = b.slots.entries('conversation.session.header.menu.item')[0]
    ...
    expect(b.slots.entries('conversation.session.header.menu.item')).toHaveLength(0)
  })
  ...
  it('re-registers after the declaring Header slot collapses and returns', async () => {
    ...
    expect(b.slots.entries('conversation.session.header.menu.item')[0]?.component).toBe(SessionLogDownloadHeaderAction)
    redeclare()
    await b.fiber.dispose()
  })
})
