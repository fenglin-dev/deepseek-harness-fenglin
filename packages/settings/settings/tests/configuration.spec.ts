...
it('imports legacy settings without consuming the shared source or overwriting profile edits', async () => {
  ...
  expect(existsSync(`${profile.patchPath}.before-settings-import`)).toBe(true)
  expect(existsSync(join(profile.dir, '.settings-yaml-import-v1'))).toBe(true)
  const backup = readFileSync(`${profile.patchPath}.before-settings-import`, 'utf8')
  await restored.fiber.dispose()
  writeFileSync(legacy, 'default-model:\n  model: later\n')
  const again = await start()
  expect(again.agentDefaultModel.currentSelection().model).toBe('legacy')
  expect(readFileSync(`${profile.patchPath}.before-settings-import`, 'utf8')).toBe(backup)
})

it('keeps a failed legacy section for a later retry without reapplying successful sections', async () => {
  const { ctx, home, profile, start } = await fixture()
  await ctx.fiber.dispose()
  const legacy = join(home, 'settings.yaml')
  writeFileSync(legacy, 'default-model:\n  model: legacy\nmissing:\n  count: 1\n')
  const first = await start()
  await vi.waitFor(() => { expect(first.agentDefaultModel.currentSelection().model).toBe('legacy') })
  expect(existsSync(join(profile.dir, '.settings-yaml-import-v1'))).toBe(false)
  await first.fiber.dispose()
  const second = await start()
  expect(second.agentDefaultModel.currentSelection().model).toBe('legacy')
  expect(existsSync(legacy)).toBe(true)
})

it('maps legacy preset choices and external-tool connections into the declarative registry without consuming settings.yaml', async () => {
  const { ctx, home, profile, start } = await fixture({ presetRegistry: true })
  await ctx.fiber.dispose()
  const legacy = join(home, 'settings.yaml')
  ...
  const restored = await start()
  await vi.waitFor(() => {
    expect(restored.settings.describe().find(row => row.ns === 'agent-preset-registry')?.value).toMatchObject({
      selectedDefault: 'ptc', modeSelectionEnabled: false, externalTools: { codex: true, claudeCode: false },
    })
  })
  expect(readFileSync(legacy, 'utf8')).toBe(source)
  expect(parse(readFileSync(profile.patchPath, 'utf8'))).toContainEqual({
    id: 'agent-preset-registry', name: 'cordis:preset-probe',
    config: { default: 'standard', selectedDefault: 'ptc', modeSelectionEnabled: false, externalTools: { codex: true, claudeCode: false } },
  })
})
