...
/** Keep every explicit profile value while selecting only missing legacy fields. */
function missingLegacyFields(legacy: Record<string, unknown>, current: unknown): Record<string, unknown> {
  if (!isPlainObject(current)) return legacy
  const result: Record<string, unknown> = {}
  for (const [key, value] of Object.entries(legacy)) {
    if (!Object.hasOwn(current, key)) result[key] = value
    else if (isPlainObject(value) && isPlainObject(current[key])) {
      const nested = missingLegacyFields(value, current[key])
...
const LEGACY_SECTION_ENTRIES: Record<string, string> = {
  ...
  'ui-settings-navigation': 'ui-settings-general',
...
export class SettingsForms extends Service {
  ...
  /** Import legacy settings without consuming the shared source or replacing handwritten profile values.
   * A failed section leaves the marker absent, so the next startup can retry it. Already imported
   * sections have profile overrides and are therefore skipped on retry. */
  private async importLegacyDocument(): Promise<void> {
    ...
    const marker = join(profile.dir, '.settings-yaml-import-v1')
    if (existsSync(marker)) {
      const completedDigest = await readFile(marker, 'utf8')
      if (completedDigest !== digest) this.ownerContext.logger.warn('settings: legacy settings changed after import for profile %s; preserving the current profile and source file', profile.name)
      return
    }
    const parsed: unknown = parse(source)
    if (parsed !== null && !isPlainObject(parsed)) throw new TypeError('settings: legacy settings.yaml must contain an object')
    const sections = parsed ?? {}
    for (const [section, values] of Object.entries(sections)) {
      if (!isPlainObject(values)) throw new TypeError(`settings: legacy section ${section} must contain an object`)
    }
    if (existsSync(profile.patchPath)) {
      try {
        await copyFile(profile.patchPath, `${profile.patchPath}.before-settings-import`, constants.COPYFILE_EXCL)
      } catch (error) {
    ...
    for (const [section, values] of Object.entries(sections)) {
      ...
      try {
        // The first pass validated every section before any Profile write.
        const checked = values as Record<string, unknown>
        const legacyValues = section === 'agent-presets' && Object.hasOwn(checked, 'default')
          && !Object.hasOwn(checked, 'selectedDefault')
          ? { ...checked, selectedDefault: checked.default }
          : checked
        if (section === 'agent-presets') Reflect.deleteProperty(legacyValues, 'default')
        const existing = this.describe().find(row => row.ns === ns)?.user
        const pending = missingLegacyFields(legacyValues, existing)
        if (Object.keys(pending).length === 0) {
          this.ownerContext.logger.info('settings: preserving existing profile override for entry %s', ns)
          continue
        }
        await this.update(ns, pending)
      } catch (error) {
        failed = true
    ...
    this.ownerContext.logger.info('settings: imported %s into profile %s; source retained', path, profile.name)
  }
