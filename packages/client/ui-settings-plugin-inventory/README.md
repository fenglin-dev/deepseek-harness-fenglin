...
The separate **Tools & capabilities** section puts the experimental Browser Use, Computer Use, and Auto review cards first under a **New in 0.1.6** marker, followed by external product connections. Both groups use the same two-column grid and collapse to one column in a narrow settings panel. Browser Use exposes the Playwright MCP, Chrome DevTools MCP, and Stagehand choices plus isolated-launch or existing-browser attachment guidance; Computer Use exposes Cua Driver Native and MCP choices and names the operating-system permission boundary. The setup dialog resolves the current choice to a reviewed, exact-version provider and downloads it through the controlled installer, with terminal output, progress, pause, and stop controls on the card. Visible isolated Playwright/DevTools detects and reuses an installed system Chrome or Chromium executable with an isolated profile rather than downloading Chrome for Testing. Those recipes and both Computer Use choices are installed together with their shared service and written into a guarded, restart-applied Profile composition. Existing-browser attachment and Stagehand remain download-only until their endpoint or model credentials are configured, so the UI does not claim that a restart alone enables them. Auto review uses the same controlled installer and remains current-session-only after activation.
