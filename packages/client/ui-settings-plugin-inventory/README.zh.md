...
使用**插件列表**可按稳定条目 id、启用状态、来源、运行状态与禁用条件检查 Agent 预设及全局插件。诊断与恢复界面会解释受控的 Profile 操作。探索插件使用市场目录与当前 Profile 状态完成安装，不维护重复排行。外部工具可安装官方子 Agent Provider 或社区 WorkBuddy 连接插件。安装时显示 pnpm 进度，并可打开经过敏感信息清理的实时输出，不会取消安装或替换失败诊断。
...
打开**外部工具**即可安装受支持的 Codex、Claude Code Provider，或社区维护的 WorkBuddy 连接插件。WorkBuddy 会联网下载经过审核的 `dsh-workbuddy-connect@0.5.0`，并要求本机 WorkBuddy 或 WorkBuddy AI 桌面应用已经登录；该插件不随客户端预置。主按钮承载进度填充与阶段文案；安装获得 id 后，**查看下载进度**按钮启用，并在当前设置页面生命周期内于成功或失败后继续可用。关闭终端不会取消操作。用户向上滚动离开末尾时，界面暂停自动跟随；较早的受限容量输出被截断时会明确说明。
...
外部工具终端使用上一次返回的不透明字节偏移轮询 `getInstallOutput`，不会在每次状态快照中重复传输完整日志。视觉百分比只来自 `installProgress.percent`；解析、重试、安装与验证阶段保持不定进度，因此客户端不会编造加权进度。官方 Provider 请求使用桌面兼容版本清单；WorkBuddy 卡片只把已签名的客户端操作映射到经过审核的社区包规格，并继续走同一个受控联网安装器。
