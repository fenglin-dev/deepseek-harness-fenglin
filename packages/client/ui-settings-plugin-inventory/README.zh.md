...
Web 用户可以检查 agent（智能体）预设组合和全局插件清单，包括启用状态、来源、失败信息和跨两种范围的搜索。本包还展示诊断、导入插件恢复、外部工具和插件市场发现入口。新会话的探索插件控件使用市场提供的推荐与 Profile 状态执行受保护的安装，并链接到完整市场，不维护重复目录或备用统计。加载、空状态、失败和重试状态都保留在各自挂载的界面内。
