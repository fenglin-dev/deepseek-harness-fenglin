...
At 680px and below, the conversation uses dynamic viewport height and a 12px content gutter. The composer stays above the bottom safe area, uses a 16px editing size to avoid iOS focus zoom, wraps its toolbar before controls overlap, and keeps attachment and send targets at least 44px. Transcript tables, code blocks, and media remain bounded or scroll inside their own content instead of widening the page.
