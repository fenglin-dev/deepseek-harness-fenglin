...
在 680px 及以下，会话页使用动态视口高度和 12px 内容留白。输入区位于底部安全区域上方，编辑字号至少为 16px 以避免 iOS 聚焦放大，工具栏会在控件重叠前换行，附件和发送控件的触控区域至少为 44px。消息中的表格、代码块和媒体会限制在内容区域内，或在自身区域横向滚动，而不会撑宽整个页面。
