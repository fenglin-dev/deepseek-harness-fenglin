...
A settled Assistant reply also recognizes explicit PNG, JPEG, WebP, GIF, and SVG paths in links, inline file references, and plain prose. Chat resolves relative paths against the viewed Session workspace, verifies each file through the authenticated bounded file route, removes duplicates, and appends readable images in one result gallery. Fenced code examples and user messages do not trigger discovery. A file that is still being written receives bounded automatic retries; an unavailable file keeps the reply readable and exposes a manual retry. Markdown image syntax continues to render at its authored position and is not duplicated in the result gallery.
