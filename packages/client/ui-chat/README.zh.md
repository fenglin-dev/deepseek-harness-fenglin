...
已落定的 Assistant 回复还会识别链接、行内文件引用和普通文本中的明确 PNG、JPEG、WebP、GIF 与 SVG 路径。Chat 基于当前查看的 Session 工作区解析相对路径，通过经过认证且有大小限制的文件路由验证每个文件，去重后把可读图片追加到一个结果图库。代码围栏中的示例与用户消息不会触发识别。仍在写入的文件会进行有限次数的自动重试；文件不可用时不会影响回复正文，并提供手动重试。Markdown 图片语法继续在原位置渲染，不会在结果图库中重复显示。
