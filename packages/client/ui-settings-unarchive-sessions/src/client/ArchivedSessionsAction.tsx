...
export function ArchivedSessionsAction({
  activeSectionId, unarchive, useWorkspaces, t,
  ...
  const restoreAll = async (): Promise<void> => {
    ...
    setFailed(false)
    try {
      for (const sessionId of workspaces.archivedSessionIds) await unarchive(sessionId)
    } catch {
      setFailed(true)
    } finally {
      setBusy(false)
  ...
  return (
    <div className={css.action}>
      {failed && <span className={css.error} role="alert">{t('restoreFailed')}</span>}
      <Button
        variant="outline"
        size="sm"
