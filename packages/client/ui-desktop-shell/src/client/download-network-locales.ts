...
const ja = {
  ...
  'network.description': '現在はアプリ更新のみを管理します。プラグインのダウンロード設定は、公式マーケットがホスト管理のネットワークポリシーに対応した後に利用できます。',
  ...
  'network.npm.title': 'npm プラグイン', 'network.npm.description': 'npm のバージョン確認、プラグイン、依存関係に使用します。private scope とビルド拒否は維持されます。',
  'network.npm.registry.existing': '現在の registry を維持', 'network.npm.registry.npmjs': 'npm 公式', 'network.npm.registry.npmmirror': 'npmmirror',
...
const ko = {
  ...
  'network.description': '현재는 앱 업데이트만 관리합니다. 플러그인 다운로드 설정은 공식 마켓이 호스트 관리 네트워크 정책을 지원한 뒤 제공됩니다.',
  ...
  'network.npm.title': 'npm 플러그인', 'network.npm.description': 'npm 버전 조회, 플러그인 패키지와 전이 종속성에 사용합니다. 비공개 scope와 빌드 거부는 유지됩니다.',
  'network.npm.registry.existing': '현재 registry 유지', 'network.npm.registry.npmjs': '공식 npm', 'network.npm.registry.npmmirror': 'npmmirror',
...
const es = {
  ...
  'network.description': 'Actualmente solo administra las actualizaciones de la aplicación. Los controles de descarga de plugins requieren que el mercado oficial admita políticas de red gestionadas por el host.',
  ...
  'network.npm.title': 'Plugins npm', 'network.npm.description': 'Para versiones, paquetes y dependencias de npm. Se conservan los scopes privados y los bloqueos de compilación.',
  'network.npm.registry.existing': 'Mantener registry actual', 'network.npm.registry.npmjs': 'npm oficial', 'network.npm.registry.npmmirror': 'npmmirror',
...
const fr = {
  ...
  'network.description': 'Gère actuellement uniquement les mises à jour de l’application. Les réglages de téléchargement des plugins nécessitent la prise en charge des stratégies réseau gérées par l’hôte dans le marché officiel.',
  ...
  'network.npm.title': 'Plugins npm', 'network.npm.description': 'Pour les versions, paquets et dépendances npm. Les scopes privés et refus de build sont conservés.',
  'network.npm.registry.existing': 'Conserver le registry actuel', 'network.npm.registry.npmjs': 'npm officiel', 'network.npm.registry.npmmirror': 'npmmirror',
...
const de = {
  ...
  'network.description': 'Verwaltet derzeit nur Anwendungsupdates. Einstellungen für Plugin-Downloads werden verfügbar, sobald der offizielle Marktplatz hostverwaltete Netzwerkrichtlinien unterstützt.',
  ...
  'network.npm.title': 'npm-Plugins', 'network.npm.description': 'Für npm-Versionen, Plugin-Pakete und Abhängigkeiten. Private Scopes und Build-Sperren bleiben erhalten.',
  'network.npm.registry.existing': 'Bestehende Registry beibehalten', 'network.npm.registry.npmjs': 'Offizielle npm-Registry', 'network.npm.registry.npmmirror': 'npmmirror',
...
const ptBR = {
  ...
  'network.description': 'Atualmente gerencia apenas as atualizações do aplicativo. Os controles de download de plugins exigem suporte do mercado oficial a políticas de rede gerenciadas pelo host.',
  ...
  'network.npm.title': 'Plugins npm', 'network.npm.description': 'Usado para versões, pacotes e dependências do npm. Escopos privados e bloqueios de build são preservados.',
  'network.npm.registry.existing': 'Manter registry existente', 'network.npm.registry.npmjs': 'npm oficial', 'network.npm.registry.npmmirror': 'npmmirror',
