...
The community build also registers `ja`, `ko`, `es`, `fr`, `de`, `pt-BR`, and `ru` as selectable languages in one reversible effect. Feature packages own their translated namespaces, so missing coverage falls back per key to English and unloading this package removes the community catalog cleanly. The active Profile locale is reported to Electron for native menu and recovery copy; Electron's startup cache never writes the Profile preference.
