...
Source builds also show Enter recovery mode. It opens the same startup-failure page used when Harness cannot become ready, while the healthy development Harness keeps running. Retry returns to that Harness, so maintainers can exercise snapshot, data-directory, and log recovery controls without corrupting a Profile. Packaged applications neither report nor permit this capability.
