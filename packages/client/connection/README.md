...
Dedicated `connection.rpc.handle` channels belong to their registering plugin. Their HTTP routes mount through an explicit `webServer` injection when that service is available; callers only need to inject `connection`. Registration reserves the channel immediately, and disposing the caller removes its route and reservation. The same browser authentication and Host/Origin checks apply to these routes.

## Browser authentication and request trust

Every Host RPC method and WebSocket stream requires one browser session; there is no method-specific loopback tier. Each ordinary Web process mints a random launch token. `dsh-web-app` prints and opens the root URL with `?token=...`; `frontend-static` delegates root and index requests to `ctx.connection.authorizeIndex`, which accepts that token only on `GET /`, writes an authority-bound signed cookie, and redirects to clean `/`. A missing, expired, malformed, or wrong-authority cookie returns 401 before RPC dispatch. Static assets remain public. Outside NAS mode, the HTTP carrier accepts no query token beyond that root exchange and no Authorization-header token.

An explicit NAS composition also registers `/nas/health`, `/nas/pair`, and `/nas/devices`. Health is public after the Host/Origin fence. Pairing consumes one eight-digit code with a ten-minute lifetime, rotates it after use, and returns one random device credential. Only its SHA-256 hash is persisted. A valid `Authorization: Bearer` device credential authenticates the same RPC and WebSocket surface, while `/nas/devices` lists or revokes individual devices. Expired and revoked credentials fail closed. The deployment must terminate TLS before this carrier and must not publish its plaintext port.

The host adapter takes the fixed methods, routes, request parsing, and response construction from [`@deepseek-ai/dsh-nas-protocol`](../../nas/nas-protocol/README.md). Authentication, trusted-authority checks, HTTP status mapping, pairing limits, and credential persistence remain owned here.
...
Before authentication, every request still passes `src/api-request-trust.ts`. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401. `dsh web --host 0.0.0.0` remains unsupported unless the invocation explicitly enables NAS mode and supplies at least one trusted authority. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md).
