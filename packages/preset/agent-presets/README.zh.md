...
组装缺失、无法解析、不是具名插件行列表，或者引用了无法解析的模块的 preset 会被列为 broken，原因会指名出问题的行。选择器会在暂存显式选择前挂载 preset，因此插件 schema 校验或 apply 失败不会创建会话；在组装文件变化前，该失败会留在名单中。创建新会话时如果配置的默认 preset 失败，Host 会在可用时改用内置 `standard` preset。恢复已存储会话和显式指定 preset 仍然严格执行，绝不替换为其他组装。修复 preset 的文件或删除它，然后重试。
...
- **被动健康问的是「装没装」，不是「能不能 import」**——发现过程证明组装能以加载器方言解析、由具名行组成，且每一行它能证明会启动的行所引用的包装在 harness 基准之上、或所引用的文件确实存在；它从不 import 任何一个。显式选择 preset 或创建会话时才执行运行时挂载，此时的失败会在该文件版本的名单中保持可见。`disabled` 是加载器唯一会插值的条目字段，因此在该字段写了表达式的行会被跳过，而不是仅凭文件下判断。
