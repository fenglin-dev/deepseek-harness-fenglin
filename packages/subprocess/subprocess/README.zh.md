...
支持 Desktop 的本地提供方可以实现 `spawnPersistent(spec, declaration)`，用于允许在 Harness 退出后继续运行的插件服务。插件应使用 `persistentSpawnSpecFingerprint(spec)` 生成 `declaration.specFingerprint`，并提供稳定包名、精确插件版本、服务标识和面向用户的用途说明。第一次调用只会写入待确认请求并在未启动进程的情况下失败。用户在 Desktop 设置中批准这一精确的 Profile／版本／服务／启动规格组合后，插件才可用相同调用重试。命令、非秘密环境项、插件版本或服务标识变化后必须重新确认；形似秘密的环境值轮换既不会泄露到指纹，也不会使生命周期许可失效。

常驻许可只改变生命周期，不增加文件、网络、子进程或模型权限。安装、恢复、诊断和持有 Profile 修改锁的任务必须使用普通任务生命周期。插件必须能处理拒绝和稍后重试；若需在 Harness 重启后重新连接，应使用稳定的协议端点。提供方无法建立可独立恢复的进程身份时必须安全失败。
