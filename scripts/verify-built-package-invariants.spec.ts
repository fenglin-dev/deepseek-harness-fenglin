...
describe('built package invariant verifier', () => {
  ...
  it('rejects a declared runtime entrypoint that the build did not materialize', ({ task }) => {
    const { root, loaderUrl } = fixture({
      companion: false,
  ...
  it('accepts a materialized runtime entrypoint and counts each target once', ({ task }) => {
    const { root, loaderUrl } = fixture({
      companion: false,
      runtimeEntrypoint: './lib/index.js',
    })
