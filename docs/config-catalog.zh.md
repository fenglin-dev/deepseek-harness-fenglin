...
来源：[`packages/host/plugin-inventory/src/index.ts:90`](../packages/host/plugin-inventory/src/index.ts)
...
这些插件由不含 `config:` 块的 `cordis.yml` 条目加载；它们不声明配置 API。
...
抽象服务类——部署应加载其具体实现包（[能力 seam](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.zh.md)）。
