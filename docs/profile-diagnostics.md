...
| `profile.host-version-incompatible` | A package-root schema-v1 `compatibility.json` lists exact `supportedHosts` versions but excludes the running Harness | Quarantine before plugin code executes. Show current, supported, and optional recommended Host versions, then offer compatible-update discovery. Missing, malformed, oversized, symlinked, or unknown declarations do not block; broad peer ranges are not compatibility proof. |
