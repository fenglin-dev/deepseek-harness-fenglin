...
| `loader.dependency-unavailable` | A resolvable Loader module statically imports or fails while requiring another unavailable package | Attribute the failure to the unique directly enabled Bundle that declared the Loader row and isolate that root as `loader-dependency-unavailable`. A missing `@deepseek-ai/dsh-*` package indicates a DSH generation mismatch; never install an internal Host package into the Profile as an automatic repair. |
