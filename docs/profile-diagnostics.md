...
Attribution may identify a direct Profile dependency, its dependency chain, a Loader entry id, a module name, a configuration kind, or the current and declared-supported Harness versions. It never contains an absolute local path. Evidence follows the complete JavaScript `cause` chain, is limited to 8 KiB per item, and redacts Harness-home paths, user-home paths, API keys, tokens, passwords, cookies, authorization values, and DeepSeek-style secret keys. The engine keeps an unrecognized native code and evidence under `profile.unknown` rather than inventing a repair.
...
| Explicit approval | Every change to pnpm `allowBuilds`, and a user's decision to run an exact plugin version outside its valid compatibility declaration | Build approval records only the exact key and retries once. A Host-compatibility override first shows a separate risk confirmation, then records only the Profile, plugin version, Harness version, and declaration that the user approved before it runs the ordinary transactional restore. |
...
| Isolation | A valid package-owned compatibility declaration excludes the running Harness, or a failure attributable to one external root remains after safe convergence or retry | Remove the root from the active dependency and bundle composition, retain its specifier and bundle position, and expose update discovery, restore, or uninstall actions appropriate to the reason |
...
| `profile.host-version-incompatible` | A package-root schema-v1 `compatibility.json` lists exact `supportedHosts` versions but excludes the running Harness | Quarantine before plugin code executes. Show current, supported, and optional recommended Host versions, then offer compatible-update discovery. A user can acknowledge the risk and restore this exact plugin version on this exact Harness version; the approval stops matching after either version or the declaration changes. Missing, malformed, oversized, symlinked, or unknown declarations do not block; broad peer ranges are not compatibility proof. |
...
| `profile.module-resolution` | `failed to import loader entry`, `ERR_MODULE_NOT_FOUND`, `missed the module table`, a non-materialized module, or a missing package factory | Walk the complete cause chain and attribute the deepest Loader entry. When a final entry exactly matches one unique directly enabled external Bundle declaration but its bare module cannot resolve, quarantine that root immediately after installation as `loader-module-unresolvable`; user-modified or ambiguous rows open Diagnostics without automatic removal. |
| `loader.dependency-unavailable` | A resolvable Loader module imports an unavailable package or requests a named export absent from an installed dependency | Attribute the failure to the unique directly enabled Bundle that declared the deepest Loader row and isolate that root as `loader-dependency-unavailable`. Retain the dependency and missing export when Node provides them. A missing `@deepseek-ai/dsh-*` package or export indicates a DSH generation mismatch; offer removal and compatible-version discovery, but never install another internal Host version into the Profile. |
...
| `loader.lifecycle-failed` | A module imports but its uniquely owned Loader entry throws while applying or registering services | Traverse causes and aggregate errors, require an exact entry/module match to one direct external Bundle, then quarantine that proven root as `loader-lifecycle-failed`. Ambiguous ownership or a user patch remains untouched and opens Diagnostics. |
| `config.credentials-invalid` | `.credentials.yaml` parse or field type error, including a non-string `version` | Report the field path and expected type. Keep the user's credentials document unchanged and open Diagnostics when it blocks startup. |
| `config.settings-invalid` | `settings.yaml` or JSON parse failure, including `DUPLICATE_KEY` with line and column | Keep the original document untouched while the internal diagnostic Profile uses an installation-owned empty settings file. Offer opening the file or an explicit backup-and-reset operation; the latter preserves the exact original bytes before restarting Harness. |
...
An active external bundle that declares a Session peer dependency can produce `profile.session-api-incompatible` when its JavaScript iterates `session.events`. Doctor and the live inventory perform a bounded, read-only source check without executing the plugin. This warning identifies the root package and package-relative file; it does not prove the receiver type or an executed failure. It never triggers repair, isolation, or a failing Doctor exit by itself. Check for a compatible plugin update; if conversations fail, disable or uninstall that plugin through plugin management. Runtime `session.events is not iterable` errors have the same diagnostic code. Minified aliases, guarded compatibility paths and other API mismatches require runtime evidence; a clean scan is not a compatibility certificate.
...
Desktop launches set the opt-in policy that permits guarded recovery. A client module-table import failure is first matched to its Loader entry and exact direct external bundle. Because this failure happens after the Host is ready but before the client plugin tree exists, the framework-free browser kernel calls one authenticated, closed recovery Remote and keeps the loading page visible. When the Host re-validates the attribution, active manifest entry, package removal, and resulting graph, the CLI retains a retryable quarantine record and the supervisor restarts the normal Profile without that bundle. The user then enters the main application normally and finds the isolated plugin and its root cause in Diagnostics. Permanently uninstalling that inactive plugin also removes its stale lockfile importer and package residue, reconciles its repair and diagnostic records without discarding other incidents, and deletes the quarantine record last. A legacy or interrupted removal that deleted the plugin and quarantine record but left those derived references is reported as `profile.quarantine-removal-residue`; startup repair and the Diagnostics action remove only the stale metadata instead of quarantining the absent plugin again. Every other deterministic Profile, Loader, Cordis, credentials, or runtime-configuration incident writes the redacted v2 incident and emits one stable eligibility marker. The supervisor then starts one installation-owned diagnostic Profile; that Profile loads only shipped template bundles and skips external bundles and user patch layers.

The diagnostic Profile records its entry time, skipped bundle names, and whether user layers were skipped. Its bare-module anchor is the installation-maintained `$DSH_HOME/profiles/node_modules` fallback rather than the active Profile or CLI package. Desktop never navigates to that minimal Profile as an ordinary workspace: readiness opens an explicit Diagnostics mode page with plugin removal, snapshot rollback, safe data-directory switching, diagnostic export, and local log access. Retry stops the diagnostic process before starting the active Profile normally. Startup performs at most one normal attempt and one diagnostic attempt. If the installation-owned Profile also fails, the supervisor stops immediately, keeps the original Profile incident as primary evidence, and appends the diagnostic failure as secondary evidence.

Diagnostics Lab carries fixed exercises for these rules. The compatible and incompatible Host-shadow exercises install their reviewed local packages through pnpm before inspection, then restore the original workspace settings before Doctor converges or quarantines the tracked package. The `@dsh-diagnostic-lab/legacy-session-api` exercise is available in the isolated home and current Profile, remains unchecked by default, and verifies advisory `profile.session-api-incompatible` attribution without invoking the legacy helper or quarantining the package. `@dsh-diagnostic-lab/loader-dependency-unavailable` verifies post-install missing-package attribution in both an isolated home and the active Profile. The active-Profile-only `@dsh-diagnostic-lab/loader-export-unavailable` package imports a deliberately absent named export from the installed settings Host, resumes the real Harness, and verifies runtime quarantine before Diagnostics fallback. The invalid-settings exercise writes duplicate keys, waits for the real active Profile to report `config.settings-invalid` and `skippedUserSettings: true`, verifies that the original bytes remain intact, and restores the exact pre-exercise settings through **Restore all**. The test packages remain `diagnostic` resources and are never seeded during ordinary startup.

The isolated `plugin-transaction-interrupted` exercise leaves an inert candidate in `checking-startup`, then runs a separate CLI to recover its previous dependency directory and remove the completed journal. It does not stop the active Harness or alter its Profile. All lab scenarios remain unchecked by default. The existing missing-export exercise also covers plugins compiled against unavailable core APIs; it remains distinct from advisory legacy Session usage.

## Export

`dsh/profile-diagnostic-export/v1` contains the v2 issues, complete machine-readable rule catalog and version, platform, architecture, Node version, selected Profile name, diagnostic-Profile summary, quarantine records, and the current Loader entry summary. It excludes credential bodies, environment values, full diffs, absolute user paths, package-manager command interpolation, and unbounded stacks. The current export is a point-in-time support artifact, not a configuration backup and not an authorization token.
