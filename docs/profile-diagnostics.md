...
| `config.settings-invalid` | `settings.yaml` or JSON parse failure, including `DUPLICATE_KEY` with line and column | Keep the original document untouched while safe mode uses an installation-owned empty settings file. Offer opening the file or an explicit backup-and-reset operation; the latter preserves the exact original bytes before restarting Harness. |
