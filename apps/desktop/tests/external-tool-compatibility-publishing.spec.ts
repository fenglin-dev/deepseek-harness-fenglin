...
describe('external tool compatibility publication', () => {
  it('signs checked coordinates and deploys only metadata through Pages without Release write permission', async () => {
    ...
    const immutable = publish.steps.findIndex(step => step.run?.includes('Published immutable manifest differs'))
    ...
    expect(immutable).toBeGreaterThan(check)
    expect(sign).toBeGreaterThan(immutable)
    expect(sign).toBeGreaterThan(check)
    expect(upload).toBeGreaterThan(sign)
    expect(publish.steps[sign]?.with).toEqual({ 'subject-path': 'external-tools-compatibility-*.v2.json' })
    ...
    expect(commands).toContain('.artifacts/external-tool-pages/metadata/external-tools/v2')
    expect(commands).toContain('external-tools-compatibility.v2.sigstore.json')
    expect(commands).toContain('cmp --silent')
