...
Mode: this is a continuable side conversation. Your answers stay in this side thread and are viewed in the side panel; they are never delivered into the parent session.`;
		...
		*/
		function createNativeSurface(ctx, records) {
			...
			let mountedUnsubscribe;
			const onListChange = () => {
				if (mountedUnsubscribe === void 0) {
		...
		*/
		function apply(ctx) {
			...
			// Fenglin: always bind a REAL session id — terminal WS rejects "current".
			try {
			...
			// Fenglin: unify skill-center sidebar entry with task-board / SSH rows.
			try {
