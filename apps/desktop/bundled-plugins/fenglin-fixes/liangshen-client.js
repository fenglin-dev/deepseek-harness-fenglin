		...
		*/
		...
		var LeverController = class {
			...
			*/
			...
			/** Prefer live current session; never grab an unrelated started transcript. */
			resolveSessionId() {
				return this.currentSessionId() ?? this.mainBlankSessionId() ?? this.mainViewSessionId();
			}
			/**
			 * Fenglin: after the first turn there is no blank row and `list.current`
			 * may still be empty on this surface. Surface the retained mainView
			 * session so the lever shows locked instead of "no session id".
			 */
			mainViewSessionId() {
				try {
					const state = this.sessions?.list.getSnapshot();
					if (state === void 0 || state.byId === void 0) return void 0;
					const rows = Object.values(state.byId).filter(function(row) { return row != null; });
					const retained = rows.find(function(row) {
						return ((row.retainedBy && row.retainedBy.mainView || 0) > 0) && row.id !== void 0;
					});
					return retained === void 0 ? void 0 : String(retained.id);
				} catch {
					return void 0;
				}
			}
